...
function crear_editar_log($ruta_archivo,$contenido,$tipo,$ip,$referer,$useragent){
  // Definimos el catalogo de tipos de log que vamos a registrar
  $arr_tipo_log = array("[info]:","[notice]:","[warning]:","[error]:");
  // Obtenemos fecha y hora actual
  $now = DateTime::createFromFormat('U.u',microtime(true));
  // Aplicamos zona horaria necesaria
  $now->setTimeZone(new DateTimeZone('America/El_Salvador'));

  // Verificamos si existe el archivo de log
  if(file_exists($ruta_archivo)){
    // So exoste, haremos escritura en el mismo
    $archivo = fopen($ruta_archivo,"a"); // Se abre el apuntador
    // Se hace escritura en ultima linea
    ...
    // Se crea carpeta "logs"
    mkdir("logs", 777, true);
    // Se abre el apuntador
    $archivo = fopen($ruta_archivo,"w+");
    // Se hace escritura en la primera linea del archivo
    fwrite($archivo, PHP_EOL.$now->format("d-m-Y H:i:s.u T")." $ip $arr_tipo_log[$tipo] referer: $referer $contenido $useragent");
